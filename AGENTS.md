...
- 루트 `docs/index.html`은 등록된 분석 프로젝트를 연결하는 목록 페이지이며 중앙 게시 스크립트가 생성한다.
...
- 표, 실험 비교, 핵심 발견, 다음 실험의 표현 방식은 가능한 한 프로젝트 간 일관되게 유지한다.
...
- 게시 전에는 `bank/tests/test_build_portfolio.py`를 실행해 등록된 보고서 경로와 공통 탐색 링크를 검증한다.
