...
class PortfolioBuildTests(unittest.TestCase):
    ...
    def test_homepage_links_all_competitions(self):
        page = homepage_html()
    ...
    def test_publish_creates_home_and_subpages_with_assets(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("데이터 분석 대회 기록", (destination / "index.html").read_text(encoding="utf-8"))
            for slug in sources:
    ...
    def test_published_bank_report_explains_models_without_source_check_framing(self):
        ...
        self.assertIn("무엇을 예측하고, 무엇이 좋은 점수인가", report)
        self.assertIn("전체 진행 로드맵", report)
        self.assertIn("Feature를 어떻게 읽고 만들 것인가", report)
        self.assertIn("표현/변환(representation)", report)
        self.assertIn("상호작용(interaction)", report)
        self.assertIn("1.0에 가까울수록 좋고", report)
